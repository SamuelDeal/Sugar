all: sugarev sugari sugarc clean_tmp
...
clean_tmp:
	@$(RM) -rf $(OBJS) $(DEPS) src/parser.cpp src/lexer.inter.hpp src/lexer.batch.hpp src/lexer.h src/parser.hpp src/parser.output src/parser.dot test/results src/sugar.o src/sugari.o src/sugarc.o
...
src/lexer.inter.hpp: src/lexer.l
	flex --header-file=src/lexer.h --interactive -o $@ $^ 
	
src/lexer.batch.hpp: src/lexer.l
	flex --header-file=src/lexer.h --batch --fast -o $@ $^
	
src/lexer.h: src/lexer.inter.hpp

%.o: %.cpp src/config.h
	$(CC) -ggdb -c $(CPPFLAGS) -o $@ $<
	
src/sugar.o: src/lexer.batch.hpp src/parser.hpp

src/sugari.o: src/lexer.inter.hpp src/parser.hpp

src/sugarc.o: src/lexer.batch.hpp src/parser.hpp
